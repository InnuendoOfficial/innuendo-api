...
export class ReportsController {
  ...
  @ApiQuery({
    name: 'start',
    description: 'exclude all reports whose date is older than',
    example: '02/04/2022',
  })
  @ApiQuery({
    name: 'end',
    description: 'exclude all reports whose date is newer than',
    example: '08/09/2022',
  })
  ...
  getReports(@Query() queries: reportQueriesDto) {
    const userId = 1; // TODO: get the real current user_id
  ...
  @ApiForbiddenResponse({ description: 'Unauthorized Request' })
  @ApiUnprocessableEntityResponse({
    description: 'The body contains not wanted content',
  })
  @Post()
